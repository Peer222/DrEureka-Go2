...
class DataCaches:
    def __init__(self, curriculum_bins):
        from forward_locomotion_go2.go2_gym_learn.ppo.metrics_caches import DistCache, SlotCache
...
class Runner:
                ...
                for i in range(self.num_steps_per_env):
                    ...
                    if eval_expert:
                        ...
                        actions_eval = self.alg.actor_critic.act_student(obs[num_train_envs:],
                                                                         obs_history[num_train_envs:])
                    ret = self.env.step(torch.cat((actions_train, actions_eval), dim=0))  # type: ignore
    ...
    def log_checkpoint(self, it=None):
        if it is None:
            it = "final"
        else:
            it = f"{it:06d}"
        ...
        try:
            with logger.Sync():
                ...
                logger.torch_save(self.alg.actor_critic.state_dict(), f"checkpoints/ac_weights_{it}.pt")

                adaptation_module_path = str(checkpoint_path / f'adaptation_module_{it}.jit')
                ...
                body_path = str(checkpoint_path / f'body_{it}.jit')
    ...
    def log_video(self, it):
        ...
        if self.env.num_eval_envs > 0:
            ...
            if len(frames) > 0:
                ...
                logger.save_video(frames, f"videos/{it:06d}_eval.mp4", fps=1 / self.env.dt)
