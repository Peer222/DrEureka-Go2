...
if [ "$#" -ne 2 ]; then
    echo "Usage: $1 <llm-type> [open-ai/gpt-oss-20b, Qwen/Qwen3-32B-AWQ, Qwen/Qwen3-Coder-30B-A3B-Instruct-FP8, Qwen/Qwen3.5-27B-FP8] $2 <environment>"
    exit 1
fi
...
if [ "$MODEL" = "open-ai/gpt-oss-20b" ]; then
  LD_PRELOAD="$CONDA_PREFIX/lib/libstdc++.so.6" VLLM_CACHE_ROOT="/bigwork/nhwpduep/.cache" TIKTOKEN_ENCODINGS_BASE="$DATA_ROOT$MODEL/encodings" vllm serve "$DATA_ROOT$MODEL" --host $HOST --port $PORT --seed 0 --tensor-parallel-size 2 &
elif [ "$MODEL" = "Qwen/Qwen3-Coder-30B-A3B-Instruct-FP8" ]; then
  LD_PRELOAD="$CONDA_PREFIX/lib/libstdc++.so.6" VLLM_CACHE_ROOT="/bigwork/nhwpduep/.cache" vllm serve "$DATA_ROOT$MODEL" --host $HOST --port $PORT --seed 0 --gpu-memory-utilization 0.96 --max-num-seqs 16 --max-model-len 65536 --tensor-parallel-size 2 &
elif [ "$MODEL" = "Qwen/Qwen3-32B-AWQ" ]; then
  LD_PRELOAD="$CONDA_PREFIX/lib/libstdc++.so.6" VLLM_CACHE_ROOT="/bigwork/nhwpduep/.cache" vllm serve "$DATA_ROOT$MODEL" --host $HOST --port $PORT --seed 0 --gpu-memory-utilization 0.94 --max-num-seqs 16 --max-model-len 40960 --tensor-parallel-size 2 &
  ...
  LD_PRELOAD="$CONDA_PREFIX/lib/libstdc++.so.6" VLLM_CACHE_ROOT="/bigwork/nhwpduep/.cache" vllm serve "$DATA_ROOT$MODEL" --host $HOST --port $PORT --seed 0 --gpu-memory-utilization 0.96 --max-num-seqs 16 --tensor-parallel-size 2 &
fi
...
python eureka-evolved.py model=$MODEL env=$2 use_submitit=1 port=$PORT
