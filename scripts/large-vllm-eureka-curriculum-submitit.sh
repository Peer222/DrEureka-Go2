...
#SBATCH -J eureka-curriculum
#SBATCH -o slurm_logs/eureka-curriculum/%j.out
...
if [ "$#" -ne 3 ]; then
    echo "Usage: $1 <llm-type> [open-ai/gpt-oss-20b, Qwen/Qwen3-32B-AWQ, Qwen/Qwen3-Coder-30B-A3B-Instruct-FP8, Qwen/Qwen3.5-27B-FP8, Qwen/Qwen3.6-27B-FP8] $2 <environment> $3 <seed>"
    exit 1
fi
...
python eureka-curriculum.py model=$MODEL env=$2 use_submitit=1 port=$PORT seed=$3 prompt_collection=curriculum env.reward_template_file=go2_gym/rewards/eureka_reward_curriculum_template.py
