...
# TODO check plotting for target and fitness scores and ...
...
def load_prompting(cfg):
    ...
    class Prompts:
        ...
        curriculum_target_system = file_to_string(prompt_dir / "target_system.txt").format(task_description=cfg.env.description)
        curriculum_target = file_to_string(prompt_dir / ".." / "curriculum_targets" / f"{env_name}.txt")
        fitness_score = file_to_string(prompt_dir / ".." / "fitness_scores" / f"{env_name}.txt")
        # not implemented
...
def get_curriculum_target(iteration: int, cfg, database_df: pd.DataFrame) -> float:
    ...
    initial_system = (
        prompts.curriculum_target_system
        + no_think_flag
    )
    target_feedback = ""
    if len(database_df):
      iteration_dfs = database_df.groupby("iteration")
      target_feedback = "The following targets were determined and resulted in these curriculum scores:  \n"
      for iter, iteration_df in iteration_dfs:
          curriculum_target = iteration_df.iloc[0]["curriculum_target"]
          #fitness_scores = iteration_df["fitness_score"].values.tolist()
          curriculum_scores = iteration_df["curriculum_score"].values.tolist()
          target_feedback += f"{iter + 1}. Curriculum Stage:  \nGenerated Target: {curriculum_target}  \nProduced these Curriculum Scores: {curriculum_scores} with mean {np.mean(curriculum_scores):.2f}  \n\n"
    target_instruction = prompts.curriculum_target.format(target_feedback=target_feedback, curriculum_stage=str(iteration + 1))

    messages = [
        {"role": "system", "content": initial_system},
        {"role": "user", "content": target_instruction},
    ]
    ...
    ### saving messages, llm response and generated stage target
    answer = response["choices"][0]['message']  # type: ignore
    with open(f"curriculum_targets/iteration-{iteration}.md", "w") as file:
    ...
    target = parse_curriculum_target(answer["content"])
    logging.info(f"Curriculum Target: {target}")
    return target


def construct_dialog(cfg, iteration: int, database_df: pd.DataFrame, curriculum_target: float) -> Tuple[List[Dict[str, str]], List[List[int]]]:
    prompts = load_prompting(cfg)
    fitness_score_computation = prompts.fitness_score.replace("... # INSERT TARGET HERE", str(curriculum_target))
    ...
    ) + ("" if len(database_df) else fitness_score_computation)
    ...
    if len(database_df):
        topk_database: pd.DataFrame = database_df[database_df["iteration"] == iteration - 1]  # type: ignore
        if cfg.topk_sampling:
            topk_database = topk_database.sort_values(by="curriculum_score", ascending=False).iloc[:cfg.topk_sampling]
        history_samples: pd.DataFrame = topk_database.sample(1, weights="curriculum_score")

        ancestors = history_samples["ancestors"].item() + history_samples[['iteration', 'sample']].values.tolist()
        logging.info(f"Sample curriculum ancestry [i, s]: {ancestors}")

        for i, (iteration, sample) in enumerate(ancestors):
            ancestor = database_df[(database_df["iteration"] == iteration) & (database_df["sample"] == sample)]

            prefix = f"Stage {i + 1}:  \n Curriculum Target: {database_df['curriculum_target'].iloc[0].item()}; Curriculum Score: {database_df['curriculum_score'].values.tolist()}  \n"
            prefix += f"Reward:  \n```python\n"
            messages += [
                    {
                        "role": "assistant",
            ...
            feedback = prompts.policy_feedback.format(version=i + 1, epoch_freq=epoch_freq, curriculum_stage=str(iteration + 1)) + ancestor["training_summarization"].item()
            messages += [{"role": "user", "content": feedback}]

        instructions = prompts.code_feedback + prompts.code_output_tip + fitness_score_computation
        messages += [{"role": "user", "content": instructions}]
    return messages, ancestors


def generate_samples(iteration: int, curriculum_target: float, cfg, database_df: pd.DataFrame, stats):
    openai.api_key = "..."
    vllm_host = f"http://0.0.0.0:{cfg.port}"
    openai.api_base = f"{vllm_host}/v1"
    ...
    for s in range(cfg.sample):
        messages, ancestors = construct_dialog(cfg, iteration, database_df, curriculum_target)
    ...
    return responses, stats, all_ancestors


def add_failure_values(stats):
    stats["execution"].append(0)
    stats["curriculum_score_max"].append(0)
    stats["curriculum_score_mean"].append(0)
    stats["curriculum_score_min"].append(0)
...
def main(cfg):
    ...
    (workspace_dir / "curriculum_targets").mkdir()
    ...
    database_df = pd.DataFrame(columns=["iteration", "sample", "curriculum_score", "fitness_score", "generation", "reward_function", "training_summarization", "reward_code_path", "ancestors", "curriculum_target"])
    ...
    for iter in range(last_complete_iteration + 1, cfg.iteration):
        ...
        stats = {
            ...
            "curriculum_score_mean": [],
            "curriculum_score_min": [],
            "fitness_score_max": [],
            "fitness_score_mean": [],
            "fitness_score_min": [],
            "episode_length": [],
        ...
        # Get curriculum stage target
        logging.info(database_df)
        curriculum_target = get_curriculum_target(iter, cfg, database_df)
        # Get Eureka response
        samples, stats, ancestors = generate_samples(iter, curriculum_target, cfg, database_df, stats)

        # Launch all evaluations
        evaluation_runs = []
        used_gpus = []
        free_eval_gpu: int = 0
        for sample_idx, (sample, _ancestors) in enumerate(zip(samples, ancestors)):
            logging.info(f"Iteration {iter}: Processing Code Run {sample_idx}")

            checkpoint_path = None
            if len(_ancestors):
                logging.info(f"{_ancestors[-1]=}")
                (a_i, a_s) = _ancestors[-1]
                logging.info(f"{a_i=}; {a_s=}")
                glob = list(Path(f"{a_i}/").glob(f"{a_s}_*/checkpoints/ac_weights_final.pt"))
                checkpoint_path = glob[0].absolute()
            logging.info(f"{checkpoint_path=}")
            ...
            if cfg.env.env_name.lower() == "forward_locomotion_go2":
                cur_task_rew_code_string = cur_task_rew_code_string.replace(
                    "... # INSERT TARGET HERE", str(curriculum_target)
                )
            elif cfg.env.env_name.lower() == "globe_walking_go2":
                raise NotImplementedError("Curriculum target for globe walking not implemented (ball size) and environment not adapted")
            ...
            if cfg.use_submitit:
                train_cfg = {
                    ...
                    "iterations": int(cfg.env.train_iterations / cfg.iteration),
                    "reward_config": "eureka",
                    "dr_config": "off",
                    "no_wandb": not cfg.use_run_wandb,
                    # also used as result directory path
                    "wandb_group": f"eureka-curriculum/{TIMESTAMP}/{iter}/{sample_idx}",
                    "headless": True,
                    "device": "cuda:0",
                    "seed": cfg.seed,
                    "checkpoint": (checkpoint_path, int(cfg.env.train_iterations / cfg.iteration * iter)),
                }
                job = submitit_executor.submit(train, train_cfg)  # type: ignore
                evaluation_runs.append(job)
            else:
                raise NotImplementedError("Only submitit option implemented")
            ...
            if traceback_msg == "":
                ...
                for metric in sorted(run_log.keys()):
                    if metric not in ["timesteps", "iterations", "time elapsed/mean", "time iter/mean"]:
                        ...
                        elif "curriculum_score" == metric:
                            curriculum_score = max(metric_cur_max, 0)
                            stats["curriculum_score_max"].append(metric_cur_max)
                            stats["curriculum_score_mean"].append(metric_cur_mean)
                            stats["curriculum_score_min"].append(metric_cur_min)
                            metrics["curriculum_score"] = run_log[metric]
                            metric_name = "curriculum score"
                        ...
                        metrics["training_iteration"] = run_log[metric]

                stats["num_reward_functions"].append(num_rewards)
                stats["reward_names"].append(reward_names)
                iteration_metrics.append(metrics)
                logging.info(f"{iter}-{response_id}: {curriculum_score=}; {fitness_score=}")
        ...
        stats["curriculum_target"] = curriculum_target
