    ...
    if cfg.resume:
        ...
        if last_complete_iteration >= 0:
            ...
            if chat_message:
                ...
                with open(resumed_base_dir / "video_critiques" / f"iteration-{last_complete_iteration}_sample-{best_current_sample}.md", "r") as vf:
                    video_reflection = video_feedback.replace(
                        "{critique}", vf.read()
                    )
                logging.info(f"{video_reflection=}")
                messages.append({"role": "user", "content": reward_reflection + video_reflection + code_output_tip})
            else:
    ...
    for iter in range(last_complete_iteration + 1, cfg.iteration):
        ...
        for sample_idx, sample in enumerate(samples):
            ...
            if cfg.use_submitit:
                ...
                with open(rl_logpath, "w") as f:
                    ...
                    command = f"python -u {ROOT_DIR}/{env_name}/{cfg.env.train_script} --iterations {cfg.env.train_iterations} --headless --dr-config off --reward-config eureka --wandb-group v-eureka/{TIMESTAMP}/{iter}/{sample_idx} --device cuda:{free_eval_gpu} --seed {cfg.seed}"
        ...
        for response_id, evaluation_run in enumerate(evaluation_runs):
            ...
            if traceback_msg == "":
                ...
                content += construct_numeric_feedback(run_log, step_size)
